...
#define dout_subsys msgr_subsys_
...
MsgrContext *g_context;

MessageFactory<TEST_MSG_PING, MPing> ping_reg;
MessageFactory<TEST_MSG_COMMAND, MCommand> cmd_reg;
...
class MessengerTest : public ::testing::TestWithParam<const char*> {
 public:
  ...
  virtual void SetUp() {
    ...
    server_msgr = Messenger::create(g_context, string(GetParam()), entity_name_t::OSD(0), "server", getpid());
    client_msgr = Messenger::create(g_context, string(GetParam()), entity_name_t::CLIENT(-1), "client", getpid());
  ...
  virtual void TearDown() {
    delete server_msgr;
    delete client_msgr;
    std::cout << "total message: " << Message::get_total_alloc() << '\n';
    std::cout << "total conn: " << Connection::get_total_alloc() << '\n';
    std::cout << "total local conn: " << Connection::get_total_local_alloc() << '\n';
    std::cout << "total msgr " << Messenger::get_total_alloc() << '\n';
    Connection::dump_all(std::cout);
  }
...
class FakeDispatcher : public Dispatcher {
 public:
  ...
  void ms_handle_fast_connect(Connection *con) {
    ...
    cond.Signal();
    lock.Unlock();
    std::cout << __func__ << this << " session nref: " << s->get_nref() << '\n';
  }
  ...
  bool ms_dispatch(Message *m) {
    ...
    s->put();
    std::cout << __func__ << s << " session nref: " << s->get_nref() << '\n';
  ...
  bool ms_handle_reset(Connection *con) {
    ...
    if (s) {
      ...
      std::cout << __func__ << " " << s << " session nref: " << s->get_nref() - 1 << '\n';
      s->put();
    }
    return true;
  }
  void clear_session(Connection *con) {
    Mutex::Locker l(lock);
    Session *s = static_cast<Session*>(con->get_priv());
    if (s) {
      s->con.reset(NULL);  // break con <-> session ref cycle
...
TEST_P(MessengerTest, SimpleTest) {
  ...
  conn->mark_down();
  cli_dispatcher.clear_session(conn.ptr());
  ...
  ASSERT_FALSE(cli_dispatcher.got_new);
...
TEST_P(MessengerTest, StatefulTest) {
  ...
  ASSERT_FALSE(srv_dispatcher.got_remote_reset); // XXX
  conn = client_msgr->get_connection(server_msgr->get_myinst());
  CHECK_AND_WAIT_TRUE(srv_dispatcher.got_remote_reset); //XXX
  {
    m = new MPing();
    ASSERT_EQ(conn->send_message(m), 0);
    Mutex::Locker l(cli_dispatcher.lock);
  ...
  srv_dispatcher.clear_session(server_conn.ptr());
  ...
  CHECK_AND_WAIT_TRUE(conn->is_connected());
  ASSERT_TRUE(conn->is_connected());
  CHECK_AND_WAIT_TRUE(cli_dispatcher.got_remote_reset);
  {
    m = new MPing();
    ASSERT_EQ(conn->send_message(m), 0);
    ASSERT_TRUE(conn->is_connected());
...
TEST_P(MessengerTest, AuthTest) {
  g_context->conf_->set_val("auth_cluster_required", "cephx");
  g_context->conf_->set_val("auth_service_required", "cephx");
  g_context->conf_->set_val("auth_client_required", "cephx");
  FakeDispatcher cli_dispatcher(false), srv_dispatcher(true);
  entity_addr_t bind_addr;
  bind_addr.parse("127.0.0.1");
  server_msgr->bind(bind_addr);
  ...
  // 2. mix auth
  g_context->conf_->set_val("auth_cluster_required", "none");
  g_context->conf_->set_val("auth_service_required", "none");
  g_context->conf_->set_val("auth_client_required", "none");
  conn->mark_down();
  cli_dispatcher.clear_session(server_conn.ptr());
  ASSERT_FALSE(conn->is_connected());
  conn = client_msgr->get_connection(server_msgr->get_myinst());
  {
    MPing *m = new MPing();
...
class SyntheticDispatcher : public Dispatcher {
 public:
  ...
  SyntheticDispatcher(bool s): Dispatcher(g_context), lock("SyntheticDispatcher::lock"),
                          is_server(s), got_new(false), got_remote_reset(false),
  ...
  void ms_fast_dispatch(Message *m) {
    Mutex::Locker l(lock);
    if (is_server) {
      ldout(g_context, 0) << side() << __func__ << " " << m->get_connection()->get_nref() << dendl;
      reply_message(m);
    } else if (m->get_middle().length()) {
      ldout(g_context, 0) << side() << __func__ << " " << m->get_connection() << dendl;
...
TEST_P(MessengerTest, MessageTest) {
  ...
  ConnectionRef conn = client_msgr->get_connection(server_msgr->get_myinst());
  {
    ...
    Mutex::Locker l(cli_dispatcher.lock);
    while (!cli_dispatcher.got_new)
      cli_dispatcher.cond.WaitInterval(g_context, cli_dispatcher.lock, t);
 ...
 public:
  ...
  SyntheticWorkload(int servers, int clients, string type, int random_num):
    ...
    for (int i = 0; i < servers; ++i) {
      ...
      msgr = Messenger::create(g_context, type, entity_name_t::OSD(0),
                               "server", getpid()+i);
    ...
    for (int i = 0; i < clients; ++i) {
      msgr = Messenger::create(g_context, type, entity_name_t::CLIENT(-1),
    ...
    for (int i = 0; i < random_num; i++) {
      bufferlist bl;
      std::uniform_int_distribution<> u(32, max_message_len);
  ...
  ConnectionRef _get_random_connection(pair<Messenger*, Messenger*> *p) {
    ...
    std::uniform_int_distribution<> choose(0, available_connections.size() - 1);
  ...
  void drop_all_connections() {
    ...
    map<pair<Messenger*, Messenger*>, ConnectionRef>::iterator i;
    Mutex::Locker l(lock);
    for (i = available_connections.begin(); i != available_connections.end(); ++i) {
    //    i->second->mark_down();
        cli_dispatcher.clear_pending(i->second);
    }
  ...
  void generate_connection() {
    ...
    {
      std::uniform_int_distribution<> choose(0, available_servers.size() - 1);
      int index = choose(rng);
    ...
    {
      std::uniform_int_distribution<> choose(0, available_clients.size() - 1);
      int index = choose(rng);
  ...
  void send_message() {
    Message *m = new MPing();
    bufferlist bl;
    std::uniform_int_distribution<> u(0, rand_data.size()-1);
  ...
  void wait_for_done() {
    ...
    //available_connections.clear();
    std::cerr << "cli_dispatcher conn_sent:" << cli_dispatcher.conn_sent.size() << '\n';
    std::cerr << "srv_dispatcher conn_sent:" << srv_dispatcher.conn_sent.size() << '\n';
    std::cerr << "avail conn:"               << available_connections.size() << '\n';
  }
};
...
TEST_P(MessengerTest, SyntheticStressTest) {
  ...
  for (int i = 0; i < 10000; ++i) {
    ...
    std::uniform_int_distribution<> true_false(0, 99);
...
TEST_P(MessengerTest, SyntheticInjectTest) {
  ofstream out("/tmp/log.txt", std::ofstream::out);
  //g_context->conf_->set_val("ms_inject_socket_failures", "30");
  g_context->conf_->ms_inject_socket_failures = 30;
  //g_context->conf_->set_val("ms_inject_internal_delays", "0.1");
  //g_context->conf_->ms_inject_internal_delays = 0.1;
  SyntheticWorkload test_msg(4, 16, GetParam(), 100);
  //SyntheticWorkload test_msg(1, 1, GetParam(), 100);
  for (int i = 0; i < 100; ++i) {
    if (!(i % 10)) cerr << "seeding connection " << i << std::endl;
    test_msg.generate_connection();
  }
  ...
  test_msg.wait_for_done();
  //g_context->conf_->set_val("ms_inject_socket_failures", "0");
  g_context->conf_->ms_inject_socket_failures = 0;
  //g_context->conf_->set_val("ms_inject_internal_delays", "0");
  g_context->conf_->ms_inject_internal_delays = 0;
}
...
class MarkdownDispatcher : public Dispatcher {
 ...
 public:
  atomic_t count;
  MarkdownDispatcher(bool s): Dispatcher(g_context), lock("MarkdownDispatcher::lock"),
  ...
  void ms_handle_fast_connect(Connection *con) {
    cerr << __func__ << " conn:" << con << std::endl << std::flush;
    Mutex::Locker l(lock);
    conns.insert(con);
  }
  void ms_handle_fast_accept(Connection *con) {
  ...
  bool ms_handle_reset(Connection *con) {
    cerr << __func__ << " con:" << con << std::endl;
    Mutex::Locker l(lock);
    conns.erase(con);
    usleep(rand() % 500);
    return true;
...
TEST_P(MessengerTest, MarkdownTest) {
  Messenger *server_msgr2 = Messenger::create(g_context, string(GetParam()), entity_name_t::OSD(0), "server", getpid());
...
int main(int argc, char **argv) {
  ...
  g_context = new MsgrContext;
  //global_init(NULL, args, CEPH_ENTITY_TYPE_CLIENT, CODE_ENVIRONMENT_UTILITY, 0);
  //g_context->conf_->set_val("auth_cluster_required", "none");
  //g_context->conf_->set_val("auth_service_required", "none");
  //g_context->conf_->set_val("auth_client_required", "none");
  //g_context->conf_->set_val("enable_experimental_unrecoverable_data_corrupting_features", "ms-type-async");
  //g_context->conf_->set_val("ms_die_on_bad_msg", "true");
  //g_context->conf_->set_val("ms_max_backoff", "1");
  //common_init_finish(g_context);
  g_context->conf_->ms_max_backoff = 1;
  g_context->conf_->ms_async_op_threads = 2;
  //g_context->conf_->ms_die_on_bad_msg = true;

  g_context->conf_->log_level[msgr_subsys_] = 20;
  g_context->conf_->log_level[msgr_subsys_ms] = 20;
