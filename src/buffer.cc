...
#include "acconfig.h"
#include "msgr/armor.h"
#include "msgr/intarith.h"
#include "msgr/environment.h"
#include "msgr/cpp_errno.h"
#include "msgr/safe_io.h"
#include "msgr/simple_spin.h"
#include "msgr/strtol.h"
#include "msgr/compiler_extensions.h"
...
bool buffer_track_alloc = get_env_bool("MSGR_BUFFER_TRACK");
...
bool buffer_track_crc = get_env_bool("MSGR_BUFFER_TRACK");
...
bool buffer_track_c_str = get_env_bool("MSGR_BUFFER_TRACK");
...
int update_max_pipe_size()
{
    ...
    if (::stat("/proc/sys/fs/pipe-max-size", &stat_result) == -1)
        return -errno;
    r = safe_read_file("/proc/sys/fs/", "pipe-max-size", buf, sizeof(buf) - 1);
    if (r < 0)
        return r;
    buf[r] = '\0';
...
#ifdef MSGR_HAVE_SPLICE
class buffer::raw_pipe : public buffer::raw {
public:
    raw_pipe(unsigned len) : raw(len), source_consumed(0)
    {
    ...
    {
        int flags = SPLICE_F_NONBLOCK;
        ssize_t r = safe_splice(pipefds[0], NULL, fd, offset, len, flags);
        if (r < 0) {
    ...
    virtual unsigned consumed() const override {
...
private:
    ...
    unsigned source_consumed;
    int pipefds[2];
};
#endif // MSGR_HAVE_SPLICE
...
{
    return create_aligned(len, MSGR_PAGE_SIZE);
}
...
buffer::ptr::ptr(raw *r)
 : _raw(r), _off(0), _len(r->len)   // no lock needed; this is an unref raw.
{
    r->nref.inc();
...
buffer::ptr::ptr(const ptr& p)
 : _raw(p._raw), _off(p._off), _len(p._len)
{
    if (_raw) {
        _raw->nref.inc();
        bdout << "ptr " << this << " get " << _raw << bendl;
...
{
    return is_n_align_sized(MSGR_PAGE_SIZE);
}

bool buffer::list::is_page_aligned() const
{
    return is_aligned(MSGR_PAGE_SIZE);
}
...
{
    rebuild_aligned(MSGR_PAGE_SIZE);
}
...
void buffer::list::append(const char *data, unsigned len)
{
    while (len > 0) {
        ...
        // make a new append_buffer!
        unsigned alen = MSGR_PAGE_SIZE * (((len-1) / MSGR_PAGE_SIZE) + 1);
        append_buffer = create_page_aligned(alen);
        append_buffer.set_length(0);   // unused, so far.
    }
}
...
const char& buffer::list::operator[](unsigned n) const
{
    ...
    for (p = _buffers.begin(); p != _buffers.end(); ++p) {
        if (n >= p->length()) {
            n -= p->length();
            continue;
        }
...
{   // fixme?
    ...
    while (len > 0 && curbuf != _buffers.end()) {
        // partial?
        if (off + len < (*curbuf).length()) {
...
ssize_t buffer::list::read_fd(int fd, size_t len)
{
    ...
    int s = ROUND_UP_TO(len, MSGR_PAGE_SIZE);
...
ssize_t buffer::list::write_file(const char *fn, int mode)
{
...
ssize_t buffer::list::write_fd(int fd) const
{
    if (can_zero_copy())
...
ssize_t buffer::list::write_fd_zero_copy(int fd) const
{
    if (!can_zero_copy())
        return -ENOTSUP;
    ...
    for (std::list<ptr>::const_iterator it = _buffers.begin();
        it != _buffers.end(); ++it) {
        ssize_t r = it->zero_copy_to_fd(fd, off_p);
        if (r < 0) {
            if (r == -EAGAIN) {
                if (total > 0)
                    return total;
...
uint32_t buffer::list::crc32c(uint32_t crc) const
{
  for (std::list<ptr>::const_iterator it = _buffers.begin();
	...
	if (ccrc.first == crc) {
      ...
      {
	uint32_t base = crc;
	crc = msgr_crc32c(crc, (unsigned char*)it->c_str(), it->length());
...
void buffer::list::hexdump(std::ostream &out) const
{
    ...
    char original_fill = out.fill('0');
    std::streamsize original_width = out.width();
