...
public:
 ...
 public:
  ...
  utime_t(const struct msgr_timespec &v) {
    decode_timeval(&v);
  }
  ...
  void encode(bufferlist &bl) const {
    msgr::encode(tv.tv_sec, bl);
    msgr::encode(tv.tv_nsec, bl);
  }
  void decode(bufferlist::iterator &p) {
    msgr::decode(tv.tv_sec, p);
    msgr::decode(tv.tv_nsec, p);
  }

  void encode_timeval(struct msgr_timespec *t) const {
    t->tv_sec = tv.tv_sec;
    t->tv_nsec = tv.tv_nsec;
  }
  void decode_timeval(const struct msgr_timespec *t) {
    tv.tv_sec = t->tv_sec;
  ...
  std::ostream& gmtime(std::ostream& out) const {
    auto oldfmt = out.setf(std::ios::right);
  ...
  static int parse_date(const std::string& date, uint64_t *epoch,
                        uint64_t *nsec, std::string *out_date=NULL,
                        std::string *out_time=NULL) {
    struct tm tm;
